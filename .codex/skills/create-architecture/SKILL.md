...
description: "Guided, section-by-section authoring of the master architecture document for the game. Reads all GDDs, the systems index, existing ADRs, and the engine reference library to produce a complete architecture blueprint before any code is written. Engine-version-aware: separates model knowledge risk from current documentation evidence and validates decisions against the pinned engine version."
...
Read the active engine reference set. Do not recursively load directories marked
`legacy`, `historical`, `archive`, or `superseded`.

1. `.codex/docs/technical-preferences.md`
   → Extract the configured engine and project pin.
2. `docs/engine-reference/[engine]/VERSION.md`
   → Extract engine version, project pin, Model Knowledge Baseline, Knowledge
   Risk, Documentation Evidence, and Last Docs Verified.
3. `docs/engine-reference/[engine]/breaking-changes.md`
   → Extract version-specific changes relevant to project domains.
4. `docs/engine-reference/[engine]/deprecated-apis.md`
   → Extract APIs to avoid and their replacements.
5. `docs/engine-reference/[engine]/current-best-practices.md`
   → Extract current version-specific practices.
6. Active files in `docs/engine-reference/[engine]/modules/`
   → Extract evidence for each project-relevant domain.
...
If the configured project pin and active `VERSION.md` disagree, classify the
engine evidence as STALE and stop before authoring.
...
For every engine domain required by the Technical Requirements Baseline,
classify the active evidence:

- **VERIFIED** — matching engine version, verification date, official sources,
  and coverage of the required behavior or APIs.
- **PARTIAL** — engine-level references are current, but domain or API coverage
  is incomplete.
- **MISSING** — no active evidence covers the domain.
- **STALE** — evidence conflicts with the project pin or newer official facts.

Display:

```text
## Engine Evidence Inventory

Engine: [name + version]
Model Knowledge Baseline: [value or Unknown]
Knowledge Risk: [LOW / MEDIUM / HIGH]
Reference Set Last Verified: [date]

| Domain | Evidence | Version | Last Verified | Active Source | Required Action |
|--------|----------|---------|---------------|---------------|-----------------|
| Input | VERIFIED | [version] | [date] | modules/input.md | Implementation test |
| Rendering | PARTIAL | [version] | [date] | VERSION.md | Verify required APIs |
| Audio | MISSING | — | — | — | Refresh reference |
```

If every required domain is VERIFIED, display:

```text
ENGINE REFERENCES VERIFIED
All currently required engine domains have active version-matching evidence.
Continue without a generic knowledge-gap warning. Preserve implementation and
runtime verification requirements.
```

Do not pause merely because Knowledge Risk is HIGH.

If any required domain is PARTIAL, MISSING, or STALE, use `AskUserQuestion`:

- Prompt: "One or more required engine domains lack current version-specific
  evidence. How would you like to proceed?"
- Options:
  - `[A] Refresh the active engine references first`
  - `[B] Show the exact missing evidence`
  - `[C] Continue only with VERIFIED domains and leave the others unresolved`

Never make an engine-dependent architectural decision for a MISSING or STALE
domain.
...
layers, flag if its Documentation Evidence is PARTIAL, MISSING, or STALE. HIGH
Knowledge Risk with VERIFIED evidence receives an informational source note,
not a warning.
...
- **Engine APIs used**: which specific engine classes/nodes/signals this module
  calls directly, with engine version, Documentation Evidence, active source,
  and remaining implementation verification noted
...
relevant active module reference. If its evidence is not VERIFIED, flag it:

```text
⚠️ ENGINE API EVIDENCE GAP
API: [ClassName.method()]
Engine: [name + version]
Documentation Evidence: [PARTIAL / MISSING / STALE]
Active Source: [path or "None"]
Missing Evidence: [specific gap]
Required Action: [refresh reference / verify official API / avoid decision]
```
...
- [ ] Are version-sensitive APIs supported by active evidence and explicit
...
[Knowledge Risk, per-domain Documentation Evidence, active sources, and
remaining verification requirements from Phase 0d]
