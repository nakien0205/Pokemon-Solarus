---
name: setup-engine
description: "Configure the project's game engine and version. Pins the engine in CLAUDE.md, records model-knowledge risk separately from current documentation evidence, and populates engine reference docs when that evidence is missing or stale."
...
After updating CLAUDE.md, create or update `.codex/docs/technical-preferences.md` with
...
## 6. Determine Knowledge Risk and Documentation Evidence

Treat these as separate facts:

- **Project Engine Version**: the exact engine version or build pinned by the
  project.
- **Model Knowledge Baseline**: the latest engine information the current model
  is explicitly known to cover.
- **Documentation Evidence**: current official, version-specific sources that
  cover the pinned engine and relevant project domains.

Never use a skill's creation date, modification date, or an inherited hard-coded
date as the current model knowledge baseline.

If the runtime explicitly provides a model knowledge baseline, record it.
Otherwise record:

`Model Knowledge Baseline: Unknown — runtime did not expose a verified baseline`

When the baseline is unknown, use `Knowledge Risk: HIGH — model coverage
unproven`. This is conservative and does not mean the project uses an older
engine.

Classify documentation evidence independently:

- **VERIFIED** — official sources match the pinned engine release family,
  include a verification date, and cover the relevant project domains.
- **PARTIAL** — version-level sources are current, but one or more relevant
  domains or patch-specific behaviors are not covered.
- **MISSING** — no matching official version-specific sources were found.
- **STALE** — a reference targets a different engine version, is marked
  historical or superseded, or conflicts with newer official information.

A HIGH Knowledge Risk alone does not mean a skill or reference is stale.
It means current official evidence is required.

Report both values:

- `Knowledge Risk: [LOW / MEDIUM / HIGH]`
- `Documentation Evidence: [VERIFIED / PARTIAL / MISSING / STALE]`

Only `PARTIAL`, `MISSING`, or `STALE` documentation evidence should produce an
engine evidence-gap warning. `VERIFIED` evidence should produce an informational
...
### If Knowledge Risk is LOW and Documentation Evidence is VERIFIED:
...
| **Model Knowledge Baseline** | [runtime-provided value, or "Unknown — runtime did not expose a verified baseline"] |
| **Knowledge Risk** | [LOW / MEDIUM / HIGH, with reason] |
| **Documentation Evidence** | VERIFIED |
| **Last Docs Verified** | [today's date] |
| **Official Sources** | [version-specific official documentation consulted] |

## Note

Model knowledge risk and documentation evidence are independent. Continue to
verify version-sensitive APIs against official sources even when Knowledge Risk
is LOW.
...
### If Knowledge Risk is MEDIUM/HIGH or Documentation Evidence is not VERIFIED:
...
2. **Fetch and extract** from official documentation:
   - Breaking changes between the project's prior engine version, if any, and
     the pinned version
   - Deprecated APIs with replacements
   - Current version-specific features and best practices
...
3. **Create the full reference directory**:
   ...
   ├── VERSION.md              # Version pin + knowledge risk + evidence status
...
`VERSION.md` must record the Project Engine Version, Model Knowledge Baseline,
Knowledge Risk, Documentation Evidence, Last Docs Verified date, and official
sources. Do not scan, promote, or modify directories marked `legacy`,
`historical`, `archive`, or `superseded`.
...
1. Read the active `docs/engine-reference/<engine>/VERSION.md` to get the pinned
   engine version, Knowledge Risk, Documentation Evidence, and verification date.
2. Ignore directories marked `legacy`, `historical`, `archive`, or `superseded`.
3. Use official sources to check for:
   - New releases since the last verification
   - Updated migration guides or release notes
   - Newly deprecated APIs
   - Changes affecting active project domains
4. Reclassify Documentation Evidence as VERIFIED, PARTIAL, MISSING, or STALE.
5. Show all proposed reference changes and request approval before writing.
6. Update only approved active reference files and their verification dates.
7. Report what changed and which domains remain PARTIAL, MISSING, or STALE.
...
1. Update `docs/engine-reference/<engine>/VERSION.md`:
   ...
   - `Project Pinned` → today's date
   - `Last Docs Verified` → today's date
   - Re-evaluate `Knowledge Risk` without inventing a model baseline
   - Re-evaluate `Documentation Evidence` for the target version and each
...
Knowledge Risk:  [LOW/MEDIUM/HIGH]
Docs Evidence:   [VERIFIED/PARTIAL/MISSING/STALE]
