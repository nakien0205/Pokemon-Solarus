...
1. Read `.codex/docs/technical-preferences.md` for the configured engine and
   project pin.
2. Read `docs/engine-reference/[engine]/VERSION.md` to get:
   - Engine name and version
   - Project pin
   - Model Knowledge Baseline, if known
   - Knowledge Risk
   - Documentation Evidence, if already recorded
   - Last Docs Verified date
3. If the configured engine and active `VERSION.md` disagree, classify the
   evidence as `STALE` and stop before drafting.
4. Identify the decision domain from the title or user description. Common
   domains include Physics, Rendering, UI, Audio, Navigation, Animation,
   Networking, Core, Input, and Scripting.
5. Read only active references relevant to that domain:
   - `docs/engine-reference/[engine]/modules/[domain].md`, if present
   - `breaking-changes.md`
   - `deprecated-apis.md`
   - `current-best-practices.md`
6. Never read a directory marked `legacy`, `historical`, `archive`, or
   `superseded` as active guidance.

Classify domain evidence:

- **VERIFIED** — active references match the pinned engine, have a verification
  date and official sources, and cover the APIs or behavior needed by this ADR.
- **PARTIAL** — the engine version is verified, but the selected domain or a
  required API is not fully covered.
- **MISSING** — no active version-specific evidence covers the domain.
- **STALE** — active references conflict with the project pin or newer official
  information.

Knowledge Risk and Documentation Evidence are independent. A HIGH Knowledge
Risk with VERIFIED evidence is not an engine-version failure.

If evidence is VERIFIED, display:

```text
ENGINE EVIDENCE STATUS
Engine: [name + version]
Domain: [domain]
Knowledge Risk: [level] — model coverage only
Documentation Evidence: VERIFIED
Last Verified: [date]
Sources: [active official references]

Current version-specific evidence is available. Continue using these sources
and record any behavior that still requires implementation testing.
```

Do not pause or display a knowledge-gap warning solely because Knowledge Risk is
HIGH when Documentation Evidence is VERIFIED.

If evidence is PARTIAL, MISSING, or STALE, display:

```text
ENGINE EVIDENCE GAP WARNING
Engine: [name + version]
Domain: [domain]
...
- [specific missing domain, API, version, or source]

Required action:
- Refresh the active engine reference through `/setup-engine refresh`, or
- provide current official evidence before this ADR relies on the affected API.
```

For `MISSING` or `STALE`, stop before drafting an engine-dependent decision.
For `PARTIAL`, the ADR may remain `Proposed`, but its unverified assumptions
must be listed under Verification Required.
...
| **References Last Verified** | [latest relevant verification date] |
| **References Consulted** | [active version-specific references read] |
| **Version-Sensitive APIs Used** | [APIs requiring version-specific evidence, or "None"] |
